...
export function parseFlags(tokens: string[]): { flags: Record<string, string | true>; positionals: string[] } {
 ...
 */
export async function runZgitCommand(runtime: ZerogitRuntime, rawInput: string, signal: AbortSignal | undefined, workspaceRoot?: string): Promise<CommandResult> {
  try {
    const { flags, positionals } = parseFlags(tokenize(rawInput))
    const sub = positionals.shift()?.toLowerCase() ?? 'help'
    const call: CallOptions = workspaceRoot === undefined
      ? { signal }
...
async function dispatch(
  ...
  switch (sub) {
    ...
    case 'show': {
      ...
      if (result.binary) return `${result.path} @ ${result.ref}: binary file (${result.bytes} bytes)`
    ...
    case 'ls': {
      ...
      if (result.truncated) lines.push(`(truncated at ${result.entries.length} entries)`)
      return lines.join('\n')
    }
    case 'log': {
      requireArgs(sub, args, 1)
    ...
    case 'status': {
      ...
      if (result.upToDate) return `${result.dir}: up to date @ ${result.meta.sha.slice(0, 12)}`
...
function requireArgs(sub: string, args: string[], count: number): void {
