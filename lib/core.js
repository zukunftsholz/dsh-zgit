...
import { extractArchive, detectArchiveKind } from "./archive.js";
import { RepoSpecError, ZgError } from "./errors.js";
import { createWorkspace, safeDirName } from "./fs-util.js";
import { archiveUrlFor, fetchCommits, fetchCompare, fetchDefaultBranch, fetchLatestReleaseTag, fetchRawFile, fetchRefs, fetchRelease, fetchTree, parseRepoSpec, requestHeaders, resolveCommit, resolveRefSmart, } from "./forge.js";
import { httpBytes, privateBypass } from "./http.js";
...
export async function readCheckoutMeta(dirAbs) {
...
export async function lsRemote(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
    if (repo.kind === 'generic')
        throw new RepoSpecError(`host "${repo.host}" has no known API; cannot list refs`);
...
export async function cloneSource(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
    ...
    const archiveKind = args.archiveKind ?? 'tar.gz';
...
export async function showFile(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
...
export async function lsTree(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
    const { sha, ref } = await resolveRef(runtime, repo, args.ref, options.signal);
...
export async function log(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
    const { sha, ref } = await resolveRef(runtime, repo, args.ref, options.signal);
...
export async function diff(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
    const headers = headersFor(runtime, repo);
    ...
    if (base === undefined)
        throw new RepoSpecError(`no base ref given and the default branch of ${repo.owner}/${repo.repo} could not be determined`);
...
export async function status(runtime, args, options = {}) {
    const workspace = createWorkspace(options.workspaceRoot ?? runtime.workspaceRoot);
    const dirAbs = workspace.resolve(args.dir);
    const meta = await readCheckoutMeta(dirAbs);
    const repo = args.repo !== undefined
...
export async function fetchReleaseAssets(runtime, args, options = {}) {
    const repo = parseRepoSpec(args.repo);
    if (repo.kind === 'generic')
        throw new RepoSpecError(`host "${repo.host}" has no known releases API`);
...
async function verifyReleaseChecksums(dirAbs, assets) {
    ...
    for (const asset of out) {
        for (const algorithm of ['sha256', 'sha512']) {
            if (!asset.name.endsWith(`.${algorithm}`))
                continue;
...
function parseChecksumFile(content, fileName) {
    for (const line of content.split(/\r?\n/)) {
        ...
        const hex = parts[0];
        if (hex === undefined || !/^[0-9a-fA-F]{16,}$/.test(hex))
            continue;
...
export async function download(runtime, args, options = {}) {
    let parsed;
    try {
        parsed = new URL(args.url);
    }
    catch {
        throw new ZgError(`invalid URL "${args.url}"`);
    }
    if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:')
        throw new ZgError(`URL must be http(s): "${args.url}"`);
    const workspace = createWorkspace(options.workspaceRoot ?? runtime.workspaceRoot);
