...
import { cloneSource, diff, download, fetchReleaseAssets, log, lossless, lsRemote, lsTree, showFile, status, } from "./core.js";
...
const num = (description, required = false) => ({ type: 'integer', description, ...(required ? { required: true } : {}) });
const bool = (description, required = false) => ({ type: 'boolean', description, ...(required ? { required: true } : {}) });
...
export function applyZgitTools(ctx, runtime, enabled) {
    const call = (exec) => {
        ...
        ctx.tools.register(defineTool({
            ...
            output: {
                schema: obj({
                    ...
                    behindBy: { oneOf: [num('commits the remote is ahead'), { type: 'null' }] },
...
function renderLsRemote(value) {
    const lines = [
        `Refs of ${value.owner}/${value.repo} (${value.forge}, ${value.host})`,
    ];
    if (value.defaultBranch !== undefined)
        lines.push(`default branch: ${value.defaultBranch}`);
    if (value.resolvedSha !== undefined)
        lines.push(`${value.ref}: ${value.resolvedSha}`);
    if (value.latestReleaseTag !== undefined)
        lines.push(`latest release: ${value.latestReleaseTag}`);
    if (value.branches.length > 0) {
        lines.push('', `branches (${value.branches.length}):`);
        for (const branch of value.branches)
            lines.push(`  ${branch.name.padEnd(28)} ${shortSha(branch.sha)}`);
    }
    if (value.tags.length > 0) {
        lines.push('', `tags (${value.tags.length}):`);
        for (const tag of value.tags)
            lines.push(`  ${tag.name.padEnd(28)} ${shortSha(tag.sha)}`);
    }
    return lines.join('\n');
}
function renderClone(value) {
    const lines = [
    ...
    if (value.symlinksSkipped > 0)
        lines.push(`symlinks skipped (not materialized): ${value.symlinksSkipped}`);
    if (value.hardlinksCopied > 0)
        lines.push(`hardlinks copied: ${value.hardlinksCopied}`);
...
function renderLsTree(value) {
    ...
    if (value.truncated)
        lines.push(`(listing truncated at ${value.entries.length} entries)`);
    return lines.join('\n');
}
...
function renderDiff(value) {
    if (value.singleFile !== undefined) {
        ...
        if (file.patch !== undefined)
            ...
            lines.push('[no unified patch available for this file (binary or too large)]');
